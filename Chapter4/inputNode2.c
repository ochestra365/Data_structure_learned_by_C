...
//void addNode(Node* target, int data) {//기능을 일컫는다. 전위함수다. 맨앞으로 새로운 노드를 생성시키는 함수다.
//	Node* newNode=(Node*)malloc(sizeof(Node));//malloc으로 동적으로 할당한다. 그리고 그 값을 구조체 포인터에 넣는다.
//	//newNode는 다른 메모리에서 만들어진 노드이다. r밸류를 형변환해줘야 l밸류에 넣을 수 있다.
//	//해당 노드는 함수내에서 실행할 템프노드이다. 힙영역에 저장은 되겠지만 연산 자체는 스택에서 휘발해버리는 논리구조를 가지고 있는 함수라는 것을 수 있다.
//	newNode->data = data;//템프 노드는 data 매개변수를 메인 렘으로부터 받아서 데이터를 할당한다.
//	newNode->link = target->link;//타게팅되는 노드의 링크값을 새로운 노드의 link값에 할당한다.
//	target->link = newNode;//템프 노드의 시작 주소값을 link값에 넣는다. newNode의 시작주소값은 타게팅된 구조체의 데이터 시작주소값을 가리킨다.
//	//그리고 마무리 연산 후에 함수에 반환한다.
...
//	//출력이 정상적으로 되는 것을 봐서는 addNode 함수로 데이터를 정상적으로 넘겨줬음을 알 수 있다.
//	//출력파트
//	printf("%d\n",node1->data);
//	printf("%d\n", node2->data);
//	printf("%d\n", node3->data);
