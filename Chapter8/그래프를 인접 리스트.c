#include <stdio.h>
#include <stdlib.h>
#define MAX_VERTEX 30//헤드 포인터의 최대 크기
...
g->n++;//그래프 정점 개수 n을 하나 증가
}
//그래프 g에 간선 (u,v)를 삽입하는 연산
void insertEdge(graphType* g, int u, int v) {
	graphNode* node;
	//간선 (u,v)를 삽입하기 위해 정점 u와 정점 v가 현재 그래프에 있는 지 확인
	if (u >= g->n || v >= g->n) {
		printf("(\n그래프에 없는 정점입니다.");
		return;
	}
...
//그래프 g의 각 정점에 대한 읹버 리스트를 출력하는 연산
