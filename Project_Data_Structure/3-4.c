...
////해당 50 데이터값을 매크로로 설정한 것은 직관성과 가시성을 높이기 위해서 이다.
//
...
//
//int main(void) {//다항식의 계수부분은 1차원 배열이 선언되었다. 논리적으로 배열 관계를 만들어 내었다.
...
//	return 0;
//}
////근데 이게 모순인게 서식을 모르면 이렇게 저장할 수 없잖아 선언은 먼저 프린트가 와야 하고 더하는 건 나중에 와야 논리적으로 말이 맞지. 그리고 C에 대한 값을 저장할 때 C는 printf("\nC(x)="); printPoly(C);  이 실행문 바로 위에 와야 논리적 연계가 맞지
...
//	int A_degree = A.degree, B_degree = B.degree;//메인에서 복사해온 차수를 초기화된 함수 멤버변수를 addpoly함수의 지역변수에 대입한다. 복사해온 데이터를 저장해야 함수에 할당된 버퍼에서 사용할 수 있다.
...
//			A_degree--;//차수를 하나씩 낮춰서 비교해간다. A의 차수가 크다면 A를 낮춰서 다음 차수와 비교하는 것이다.
...
//		//차수를 단순히 낮춰준 것은 물리적으로 값을 하나 줄인 거지만 논리적으로는 다항식 구조체의 논리적 시퀀스에서 우측으로 하나 이동한 것을 의미한다.
...
//		printf("%3.0fx^%d", P.coef[i], degree--);//%3.0f 변환문자가 의미하는 바는 숫자이다.//구조체의 2번체 멤버가 배열이기 때문에 사용할 수 있따. 차수를 하나 낮춰주고
//		if (i < P.degree) printf(" +");//반복제어 변수가 복사한 구조체의 degree멤버의 데이터 값보다 작다면 +을 붙인다.
//		//i 가 넘어가게 된다면 반복문을 빠져 나가게 된다. 낮춰주기 전의 차수에서 논리적 우항에 +을 출력하고
//	}//다음 차수에 대한 논리적 연산을 한다.
//	printf("\n");//가시성을 위한 사용자 편의 실행문
//}
////구현파트는 버퍼, 알고리즘은 수학이다.
////출력을 해주기 위해서는 A와 B가 초기화 되어야 하고 C가 마지막으로 A와 B를 더한 반환값이 저장되어야 한다.
////다항식이 일련의 순차적 자료구조이다. 다만 일반적인 배열이나 순차적 자료구조와 다른 점은 첫 인덱스가 최고 차수 즉, 전체를 억제하는 한정자라는 것이다.
////로직으로 다항식에 관한 논리적 순차 자료구조관계를 만들어 냈다.
